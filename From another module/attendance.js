...
function calculateHourlyRate(emp) {
  ...
  // Daily standard hours = Standard Workweek Hours / 7 days
  // If Core Working Hours span equals daily standard and > 8 hrs, lunch break wasn't accounted for
  const scheduleSpan = getScheduleSpanHours(emp?.coreWorkingHours);
  const dailyFromWeekly = standard / 7;
  let daily = dailyFromWeekly;
  if (scheduleSpan !== null && scheduleSpan === dailyFromWeekly && scheduleSpan > 8) {
    daily = dailyFromWeekly - 1; // Subtract 1 hour for lunch
  }
...
function calculateMonthlySummary(records, emp) {
  ...
  const scheduleSpan = getScheduleSpanHours(emp?.coreWorkingHours);
  const dailyFromWeekly = standard / 7;
  let dailyStd = dailyFromWeekly;
  if (scheduleSpan !== null && scheduleSpan === dailyFromWeekly && scheduleSpan > 8) {
    dailyStd = dailyFromWeekly - 1; // Subtract 1 hour for lunch
  }
  const tol = 0.01;
  // Display dailyFromWeekly (10h) but calculate using dailyStd (9h effective)
  const sum = { totalDays: safeRecords.length, presentDays:0, absentDays:0, overtimeDays:0, shortHoursDays:0, totalHours:0, totalOvertime:0, totalOvertimePay:0, dailyStandardHours: dailyFromWeekly, fullDays:0, leaveUsed:0, invalidDays:0 };
...
async function updateTodaySummaryBar() {
  ...
  // Create a set of valid employee IDs from the employee directory
  const validEmployeeIds = new Set();
  allEmployees.forEach(emp => {
    if (emp.employeeId) validEmployeeIds.add(emp.employeeId);
    if (emp.id) validEmployeeIds.add(emp.id);
  });
  
  // Use todayAttendanceMap to avoid counting duplicates
  // Each employee should only be counted once
  // Only count employees that exist in the employee directory
  todayAttendanceMap.forEach((r, empId) => {
    // Skip if employee is not in the employee directory (prevents counting orphan records)
    if (validEmployeeIds.size > 0 && !validEmployeeIds.has(empId)) {
      console.log('[Summary] Skipping attendance record for unknown employee:', empId);
      return;
    }
    ...
    // Check for leave - check leaveType field first, then remarks
    if ((leaveType && leaveType !== 'None' && leaveType !== '') || remarks.includes('leave') || remarks.includes('on leave')) {
      onLeave++;
...
async function loadEmployeesForAttendance() {
  ...
  // Expose employeesMap globally for audit log to access employee names
  window.employeesMap = employeesMap;
...
function getTodayAttendanceStatus(emp) {
  ...
  // Check for undertime (less than standard hours)
  const standardWeeklyHours = emp.standardWorkweekHours || 40;
  const scheduleSpan = getScheduleSpanHours(emp?.coreWorkingHours);
  const dailyFromWeekly = standardWeeklyHours / 7;
  let standardHours = dailyFromWeekly;
  if (scheduleSpan !== null && scheduleSpan === dailyFromWeekly && scheduleSpan > 8) {
    standardHours = dailyFromWeekly - 1; // Subtract 1 hour for lunch
  }
...
function renderAttendanceModal(records, employeeName, employee) {
  ...
  if (sumEl) {
    ...
    // Check if lunch adjustment applies for display
    const empStandardWeeklyHours = Number(emp?.standardWorkweekHours) || 40;
    const empScheduleSpan = getScheduleSpanHours(emp?.coreWorkingHours);
    const empDailyFromWeekly = empStandardWeeklyHours / 7;
    const hasLunchAdjustment = empScheduleSpan !== null && empScheduleSpan === empDailyFromWeekly && empScheduleSpan > 8;
    // For display: add 1 hour per day back if lunch adjustment applies
    const displayTotalHours = hasLunchAdjustment ? summary.totalHours + summary.totalDays : summary.totalHours;
    ...
    // Build lunch adjustment badge HTML
    const lunchAdjustmentBadgeHTML = hasLunchAdjustment ? `
      <div style="margin-top:.5rem; display:inline-flex; align-items:center; gap:.4rem; padding:.35rem .6rem; background:linear-gradient(135deg, #ca8a04 0%, #a16207 100%); border-radius:6px; border:1px solid #eab308;">
        <span style="font-size:.9rem;">🍽️</span>
        <span style="color:#fff; font-weight:600; font-size:.75rem;">1-Hour Lunch Break Included</span>
        <span style="color:rgba(255,255,255,0.8); font-size:.7rem;">(Schedule: ${empScheduleSpan}h | Actual: ${empScheduleSpan - 1}h)</span>
      </div>
    ` : '';
    ...
    sumEl.innerHTML = `
      <div class="card" style="margin-bottom:.5rem;">
        ...
        <div style="margin-top:.6rem; display:grid; grid-template-columns: repeat(9, 1fr); gap:.5rem;">
          ...
          <div class="summary-item"><div class="muted">Total Hours</div><div class="value">${formatHoursMinutes(displayTotalHours)}</div></div>
  ...
  const rows = uniqueRecords
    ...
    .map(r => {
      ...
      const empScheduleSpan = getScheduleSpanHours(employee?.coreWorkingHours);
      const empDailyFromWeekly = empStandardWeeklyHours / 7;
      let empDailyStandardHours = empDailyFromWeekly;
      const hasLunchAdjustment = empScheduleSpan !== null && empScheduleSpan === empDailyFromWeekly && empScheduleSpan > 8;
      if (hasLunchAdjustment) {
        empDailyStandardHours = empDailyFromWeekly - 1; // Subtract 1 hour for lunch (for calculations)
      }
      
      // For display: add 1 hour back if lunch adjustment applies (to show schedule hours, not worked hours)
      const displayTotalVal = hasLunchAdjustment ? totalVal + 1 : totalVal;
      const total = formatHoursMinutes(displayTotalVal);
...
window.openEditAttendance = async function(id) {
  ...
  // Show/Hide lunch adjustment badge
  const editLunchBadge = document.getElementById('editLunchAdjustmentBadge');
  if (editLunchBadge) {
    const empStandardWeeklyHours = Number(employee?.standardWorkweekHours) || 40;
    const empScheduleSpan = getScheduleSpanHours(employee?.coreWorkingHours);
    const empDailyFromWeekly = empStandardWeeklyHours / 7;
    const hasLunchAdjustment = empScheduleSpan !== null && empScheduleSpan === empDailyFromWeekly && empScheduleSpan > 8;
    if (hasLunchAdjustment) {
      editLunchBadge.style.display = 'block';
      const scheduleDisplay = document.getElementById('editScheduleDisplay');
      const actualWorkDisplay = document.getElementById('editActualWorkDisplay');
      if (scheduleDisplay) scheduleDisplay.textContent = `${empScheduleSpan} hrs`;
      if (actualWorkDisplay) actualWorkDisplay.textContent = `${empScheduleSpan - 1} hrs`;
    } else {
      editLunchBadge.style.display = 'none';
    }
  }
...
// =====================================================
// DIAGNOSE & RECALCULATE ATTENDANCE RATES
// =====================================================

// Store diagnosis results for fixing
let diagnosisResults = [];

// Show the diagnose modal
window.showDiagnoseAttendanceModal = function() {
  console.log('[Diagnose] showDiagnoseAttendanceModal called');
  console.log('[Diagnose] currentEmployeeId:', currentEmployeeId);
  
  const modal = document.getElementById('diagnoseAttendanceModal');
  console.log('[Diagnose] modal element:', modal);
  if (!modal) {
    console.error('[Diagnose] Modal element not found!');
    showAttendanceNotification('Diagnose modal not found. Please refresh the page.', 'error');
    return;
  }
  
  // Get current employee info
  const employee = employeesMap.get(currentEmployeeId);
  console.log('[Diagnose] employee:', employee);
  if (!employee) {
    console.warn('[Diagnose] No employee selected');
    showAttendanceNotification('Please select an employee first.', 'error');
    return;
  }
  ...
  document.getElementById('diagnoseEmpName').textContent = currentEmployeeName || '-';
  document.getElementById('diagnoseEmpSalary').textContent = `₱${baseSalary.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`;
  document.getElementById('diagnoseExpectedRate').textContent = `₱${hourlyRate.toFixed(2)}/hr`;
  
  // Reset state
  document.getElementById('diagnoseLoading').style.display = 'none';
  document.getElementById('diagnoseResults').style.display = 'none';
  document.getElementById('diagnoseFixBtn').style.display = 'none';
  document.getElementById('diagnoseRunBtn').style.display = 'inline-flex';
  diagnosisResults = [];
  
  modal.style.display = 'block';
};

// Close diagnose modal
window.closeDiagnoseAttendanceModal = function() {
  const modal = document.getElementById('diagnoseAttendanceModal');
  if (modal) modal.style.display = 'none';
};

// Run the diagnosis
window.runAttendanceDiagnosis = async function() {
  const loadingEl = document.getElementById('diagnoseLoading');
  const resultsEl = document.getElementById('diagnoseResults');
  const runBtn = document.getElementById('diagnoseRunBtn');
  const fixBtn = document.getElementById('diagnoseFixBtn');
  ...
  try {
    ...
    const allRecords = await getAll();
    const employeeRecords = allRecords.filter(r => r.employeeId === currentEmployeeId);
    ...
    // Check if employee is fixed rate
    const rateType = (employee.rateType || '').toLowerCase();
    const empType = (employee.employmentType || employee.type || '').toLowerCase();
    const isFixed = rateType.includes('fixed') || rateType.includes('monthly') || rateType.includes('salary') || empType.includes('fixed');
    ...
    for (const record of employeeRecords) {
      const totalHours = Number(record.totalHoursWorked) || 0;
      const currentOT = Number(record.overtimeHours) || 0;
      const currentOTPay = Number(record.overTimePay) || 0;
      
      // Calculate expected overtime
      const expectedOT = isFixed ? 0 : Math.max(0, totalHours - dailyStandardHours);
      const expectedOTPay = isFixed ? 0 : (expectedOT * hourlyRate * 1.25);
      
      // Check if values match (with tolerance)
      const otMatch = Math.abs(currentOT - expectedOT) < 0.01;
      const otPayMatch = Math.abs(currentOTPay - expectedOTPay) < 0.01;
      const isCorrect = otMatch && otPayMatch;
    ...
    diagnosisResults.sort((a, b) => new Date(b.date) - new Date(a.date));
    
    // Update summary
    document.getElementById('diagnoseTotalRecords').textContent = employeeRecords.length;
    document.getElementById('diagnoseCorrectCount').textContent = correctCount;
    document.getElementById('diagnoseNeedUpdateCount').textContent = needUpdateCount;
    
    const statusEl = document.getElementById('diagnoseStatus');
    if (needUpdateCount === 0) {
      statusEl.innerHTML = '<span style="color:#28a745;">✓ All Good</span>';
      fixBtn.style.display = 'none';
    } else {
      statusEl.innerHTML = '<span style="color:#f59e0b;">⚠ Issues Found</span>';
      fixBtn.style.display = 'inline-flex';
    }
    
    // Render issues table
    const tbody = document.getElementById('diagnoseIssuesBody');
    tbody.innerHTML = diagnosisResults.map(r => {
      const dateFormatted = new Date(r.date).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' });
      const statusIcon = r.isCorrect 
        ? '<span style="color:#28a745;">✓</span>' 
        : '<span style="color:#f59e0b;">⚠</span>';
      const rowBg = r.isCorrect ? '' : 'background:rgba(245,158,11,0.1);';
      
      return `
        <tr style="${rowBg}">
          <td style="padding:0.5rem 0.75rem;">${dateFormatted}</td>
          <td style="padding:0.5rem 0.75rem; text-align:right;">${r.totalHours.toFixed(2)}h</td>
          <td style="padding:0.5rem 0.75rem; text-align:right; ${!r.otMatch ? 'color:#f59e0b;' : ''}">${r.currentOT.toFixed(2)}h</td>
          <td style="padding:0.5rem 0.75rem; text-align:right; color:#0dcaf0;">${r.expectedOT.toFixed(2)}h</td>
          <td style="padding:0.5rem 0.75rem; text-align:right; ${!r.otPayMatch ? 'color:#f59e0b;' : ''}">₱${r.currentOTPay.toFixed(2)}</td>
          <td style="padding:0.5rem 0.75rem; text-align:right; color:#0dcaf0;">₱${r.expectedOTPay.toFixed(2)}</td>
          <td style="padding:0.5rem 0.75rem; text-align:center;">${statusIcon}</td>
        </tr>
      `;
    }).join('');
    ...
    showAttendanceNotification('Error running diagnosis: ' + err.message, 'error');
    loadingEl.style.display = 'none';
  } finally {
    runBtn.disabled = false;
    runBtn.innerHTML = '🔍 Run Diagnosis';
  }
};

// Fix all attendance rate issues
window.fixAttendanceRates = async function() {
  const fixBtn = document.getElementById('diagnoseFixBtn');
  const originalText = fixBtn.innerHTML;
  
  // Get records that need updating
  const recordsToFix = diagnosisResults.filter(r => !r.isCorrect);
  
  if (recordsToFix.length === 0) {
    showAttendanceNotification('No issues to fix', 'info');
    return;
  }
  
  // Confirm action
  if (!confirm(`This will update ${recordsToFix.length} attendance record(s) with the correct overtime calculations. Continue?`)) {
    return;
  }
  ...
  try {
    ...
    for (const record of recordsToFix) {
      try {
        await update(record.id, {
          overtimeHours: record.expectedOT,
          overTimePay: record.expectedOTPay
        });
        successCount++;
      } catch (err) {
    ...
    // Show result
    if (failCount === 0) {
      showAttendanceNotification(`Successfully updated ${successCount} attendance record(s).`, 'success');
    } else {
      showAttendanceNotification(`Updated ${successCount} record(s). Failed to update ${failCount}.`, 'warning');
    }
    
    // Close modal and refresh
    window.closeDiagnoseAttendanceModal();
    ...
    showAttendanceNotification('Error fixing attendance: ' + err.message, 'error');
  } finally {
